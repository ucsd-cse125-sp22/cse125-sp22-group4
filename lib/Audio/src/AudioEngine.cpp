...
void Implementation::Update() {
	// Destroy Channels no longer playing
...
int CAudioEngine::getEventCount() {
	int count;
	sgpImplementation->mBanks.begin()->second->getEventCount(&count);
	return count;
}

char* CAudioEngine::getEventPath() {
	int count;
	sgpImplementation->mBanks.begin()->second->getEventCount(&count);
	FMOD::Studio::EventDescription* eventList[512];
	sgpImplementation->mBanks.begin()->second->getEventList(&*eventList, 512, &count);
